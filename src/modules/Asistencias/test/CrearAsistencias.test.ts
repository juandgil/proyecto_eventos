...
describe('Crear Asistencia', () => {
    ...
    it('Debe crear una asistencia correctamente', async () => {
        const data: ICrearAsistenciaIn = {
            id_usuario: 1,
            id_evento: 1,
        };
        ...
        expect(response.response.data?.ok).toBe('Asistencia creada exitosamente');
        expect(response.response.data?.data).toHaveProperty('id_asistencia');
        expect(response.response.data?.data.id_usuario).toBe(1);
        expect(response.response.data?.data.id_evento).toBe(1);
    });

    it('Debe fallar al crear una asistencia duplicada', async () => {
        const data: ICrearAsistenciaIn = {
            id_usuario: 1,
            id_evento: 1,
        };
