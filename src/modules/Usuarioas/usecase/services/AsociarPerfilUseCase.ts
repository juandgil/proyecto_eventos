...
export default class AsociarPerfilUseCase {
    ...
    async execute(data: IAsociarPerfilIn, idUsuario: number): Promise<void> {
        const consultarUsuario = await this.usuariosRepository.consultarUsuarioPorIdUsuario(idUsuario);
        if (!consultarUsuario) {
        ...
        await this.usuariosRepository.asociarPerfil(data.perfil, idUsuario);
    }
}
